...
${_prevView==='map'?'<button onclick="setView(\'map\')" style="margin-bottom:0.5rem;background:none;border:1px solid #1e3a8a;color:#1e3a8a;border-radius:6px;padding:0.3rem 0.7rem;font-size:0.8rem;cursor:pointer;">← Back to Map</button>':''}
...
${associatedPractices.length > 0 ? associatedPractices.map(pr => `<span style="cursor:pointer;text-decoration:underline;color:#1e3a8a;" onclick="event.stopPropagation();editPracticeFromProfile('${pr.id}')">${pr.name}</span>`).join(' | ') : 'No practice assigned'}
</div>
<div class="profile-meta">
${isStaffSpecialty(p.specialty)
  ? `${p.title?mi('Role',p.title):mi('Role','Office Staff')}${p.email?`<div class="meta-item" style="grid-column:1/-1"><div class="meta-label">Email</div><div class="meta-value" style="word-break:break-all;"><a href="mailto:${p.email}" style="color:#1e3a8a;">${p.email}</a></div></div>`:''}<div class="meta-item"><div class="meta-label">Type</div><div class="meta-value" style="color:#7c3aed;font-weight:600;">Staff Contact</div></div>`
  : `<div class="meta-item"><div class="meta-label">Priority Tier</div><div class="meta-value"><select onchange="quickSavePriority(this.value)" style="font-size:0.85rem;font-weight:700;padding:0.2rem 0.4rem;border:2px solid #1e3a8a;border-radius:6px;color:#1e3a8a;background:white;width:auto;cursor:pointer;">${['','1','2','3','4','5'].map(v=>v===''?`<option value="" ${!normPriority(p.priority)?'selected':''}>— set tier —</option>`:`<option value="${v}" ${normPriority(p.priority)===v?'selected':''}>P${v}</option>`).join('')}</select></div></div>${mi('Specialty',p.specialty||'Not set')}${mi('Degree',p.degree||'—')}${p.title?mi('Title',p.title):''}${p.email?`<div class="meta-item" style="grid-column:1/-1"><div class="meta-label">Email</div><div class="meta-value" style="word-break:break-all;"><a href="mailto:${p.email}" style="color:#1e3a8a;">${p.email}</a></div></div>`:''}${p.mobile_phone?`<div class="meta-item"><div class="meta-label">Mobile</div><div class="meta-value"><button onclick="startCallSession('${p.id}',null,'${p.mobile_phone.replace(/'/g,'')}')" style="background:none;border:none;color:#1e3a8a;font-weight:600;font-size:0.9rem;cursor:pointer;padding:0;-webkit-tap-highlight-color:transparent;touch-action:manipulation;">${fmtPhone(p.mobile_phone)}</button></div></div>`:''}${mi('Academic Connection',p.academic_connection||p.um_connection||'None')}${mi('Projected Volume',p.proj_vol||p.mohs_volume||'Unknown')}`}
...
<h3>${isStaffSpecialty(p.specialty)?'Staff Contact':'Provider Profile'}</h3>
<div>
...
${ci('📱','Mobile Phone',p.mobile_phone?`<button onclick="startCallSession('${p.id}',null,'${p.mobile_phone.replace(/'/g,'')}');" style="background:none;border:none;color:#1e3a8a;cursor:pointer;padding:0;font-size:0.9rem;font-weight:500;font-family:inherit;-webkit-tap-highlight-color:transparent;touch-action:manipulation;">${fmtPhone(p.mobile_phone)}</button>`:'')}
...
try {
const {error} = await db.from('providers').update({priority}).eq('id', currentPhysician.id);
...
<div class="contact-grid">${(()=>{const pLocs=practiceLocations.filter(l=>l.practice_id===p.id);const pEmail=pLocs.map(l=>l.practice_email).find(Boolean)||'';const pPhone=pLocs.map(l=>l.phone).find(Boolean)||'';const pPhoneLoc=pLocs.find(l=>l.phone)||null;const pFax=pLocs.map(l=>l.fax).find(Boolean)||'';const pHours=pLocs.map(l=>l.office_hours).find(Boolean)||'';const pStaff=pLocs.map(l=>l.office_staff).find(Boolean)||'';const pBestDays=pLocs.map(l=>l.best_days).find(Boolean)||'';const wSite=p.website?`<a href="${p.website.match(/^https?:\/\//)?p.website:'https://'+p.website}" target="_blank" style="color:#1e3a8a;">${p.website}</a>`:'';const practiceEmail=p.email||'';return (wSite?ci('🌐','Website',wSite):'')+ci('✉️','Email',practiceEmail?`<a href="mailto:${practiceEmail}">${practiceEmail}</a>`:pEmail?`<a href="mailto:${pEmail}">${pEmail}</a>`:'')+ci('📞','Phone',pPhone?`<button onclick="startCallSession(null,'${pPhoneLoc?.id||''}','${pPhone.replace(/'/g,'')}')" style="background:none;border:none;color:#1e3a8a;cursor:pointer;padding:0;font-size:0.9rem;font-weight:500;font-family:inherit;-webkit-tap-highlight-color:transparent;touch-action:manipulation;">${fmtPhone(pPhone)}</button>`:'')+ci('📠','Fax',pFax?fmtPhone(pFax):'')+ci('🕐','Office Hours',pHours)+ci('👥','Office Staff',pStaff)+ci('📅','Best Days',pBestDays)+ci('📝','Notes',p.general_notes)||(!pEmail&&!practiceEmail&&!p.general_notes&&!pPhone?'<div class="empty-notice">No details for this practice</div>':'');})()}</div>
</div>
<div class="section">
<div class="section-header">
<h3>Locations</h3>
...
const isStaffP = isStaffSpecialty(phys.specialty);
...
$('mainContent').innerHTML = `
<div class="profile-header">
<div style="font-size:0.85rem;margin-bottom:0.5rem;"><span style="cursor:pointer;color:#1e3a8a;text-decoration:underline;" onclick="viewPractice('${practice?.id}')">← ${practice?.name || 'Practice'}</span></div>
...
const isStaffL = isStaffSpecialty(phys.specialty);
return `
...
physIds.forEach(pid => {
entries.push({provider_id:pid,contact_date:dateVal,author:authorVal,notes:baseNote,practice_location_id:locVal});
});
// Office staff entry always saves with null provider_id
if (hasOfficeStaff) {
const staffPrefix = staffName ? `[Office Staff: ${staffName}] ` : '[Office Staff] ';
const staffNote = tv ? `[${tv}] ${staffPrefix}${nv}` : `${staffPrefix}${nv}`;
entries.push({provider_id:null,contact_date:dateVal,author:authorVal,notes:staffNote,practice_location_id:locVal});
}
// If nothing selected, save as pure location-level note (provider_id = null)
if (entries.length === 0) {
entries.push({provider_id:null,contact_date:dateVal,author:authorVal,notes:baseNote,practice_location_id:locVal});
}
...
let entries;
if (physIds.length > 0) {
entries = physIds.map(pid => ({ provider_id: pid, contact_date: dateVal, author: authorVal, notes: noteText, practice_location_id: locId }));
} else {
entries = [{ provider_id: null, contact_date: dateVal, author: authorVal, notes: noteText, practice_location_id: locId }];
}
const { error } = await db.from('contact_logs').insert(entries);
if (error) throw error;
await Promise.all(physIds.map(pid => db.from('providers').update({ last_contact: dateVal }).eq('id', pid)));
